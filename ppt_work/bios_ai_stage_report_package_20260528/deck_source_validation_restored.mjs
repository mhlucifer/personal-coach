...
function createSmbiosValidationFlowSlide(presentation, page) {
  ...
  card(slide, 72, 226, 248, 124, { fill: C.panel });
  text(slide, "人工刷写 + 手动比对", 100, 250, 180, 24, { size: 19, bold: true, color: C.ink });
  text(slide, "环境切换、dmidecode 执行、日志整理依赖人工串联。", 100, 290, 174, 40, { size: 14, color: C.slate });

  sectionLabel(slide, "新方式", 72, 400, C.blue);
  card(slide, 72, 438, 248, 142, { fill: C.softBlue });
  text(slide, "opencode + skills", 100, 462, 180, 24, { size: 19, bold: true, color: C.ink });
  text(slide, "将上传、校验、刷写、采集、报告生成串成可重复流程。", 100, 502, 174, 44, { size: 14, color: C.slate });

  const steps = ["BIOS 上传", "SHA256 校验", "远程刷写", "人工确认", "系统侧采集", "报告生成"];
  steps.forEach((s, i) => {
    const y = 178 + i * 66;
    rect(slide, 374, y + 11, 24, 24, { fill: i === 3 ? C.amber : C.blue });
    text(slide, String(i + 1), 374, y + 15, 24, 16, { size: 12, bold: true, color: "#FFFFFF", align: "center", font: MONO });
    text(slide, s, 420, y + 7, 210, 30, { size: 19, bold: true, color: C.ink });
    if (i < steps.length - 1) rect(slide, 386, y + 39, 1, 30, { fill: C.faint });
  });

  evidence(slide, images.validationLog, 680, 174, 300, 430, "");
  evidence(slide, images.validationDmi, 998, 232, 190, 300, "");
  evidenceCaption(slide, "验证日志与 dmidecode 输出可回看，便于复盘同类 SMBIOS 显示问题。", 680, 632, 508);
  return slide;
}
