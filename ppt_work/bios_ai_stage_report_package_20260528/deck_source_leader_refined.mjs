...
async function createOriginalSlide(presentation, _ctx, number) {
  ...
  if (number === 12) {
    ...
    text(slide, "不做泛化 BIOS Bug 自动修复；OS IP 变化、网络不可达、网卡异常、启动失败、硬件链路异常仍以人工处理为主。", 178, 565, 858, 24, {
      size: 16,
      color: "#FFFFFF",
    });
...
function createCloseSlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "SUMMARY", "本阶段完成客户交付，并把 SMBIOS 开发场景沉淀为可复用的 AI 工程流程", page);
  const cards = [
    ["客户交付闭环", "V3 遗留问题、V5 新增问题、Type2 SKU 和 Type11 TDX 完成阶段交付。", C.blue],
    ["规则复用沉淀", "Type9 将多版型差异收敛到显示规则，后续新增版型减少主流程改动。", C.green],
    ["AI 流程落地", "SpecCode、opencode skills、自动验证和 AI Review 形成可复用路径。", C.amber],
  ];
  cards.forEach((c, i) => {
    const x = 92 + i * 356;
    rect(slide, x, 220, 300, 220, { fill: C.panel });
    rect(slide, x, 220, 300, 8, { fill: c[2] });
    text(slide, c[0], x + 28, 260, 200, 28, { size: 24, bold: true, color: C.ink });
    text(slide, c[1], x + 28, 324, 230, 70, { size: 17, color: C.slate });
  });
  rect(slide, 92, 524, 1012, 58, { fill: C.ink });
  text(slide, "下一步", 122, 543, 82, 22, { size: 18, bold: true, color: "#FFFFFF" });
  text(slide, "聚焦 SMBIOS 可观测问题，探索验证发现、Agent 辅助定位、人工确认和复测回归路径。", 220, 543, 760, 24, {
    size: 18,
    color: "#FFFFFF",
  });
  return slide;
