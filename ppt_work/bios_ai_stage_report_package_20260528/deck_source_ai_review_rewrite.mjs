...
const images = {
  ...
  aiReviewTrace: `${PACKAGE_ASSET}/review_log_evidence/image1.png`,
  aiReviewDecision: `${PACKAGE_ASSET}/review_log_evidence/image2.png`,
  aiReviewFinal: `${PACKAGE_ASSET}/review_log_evidence/image3.png`,
  aiReviewFormal: `${PACKAGE_ASSET}/review_log_evidence/image4.png`,
  aiReviewCandidate: `${PACKAGE_ASSET}/review_log_evidence/image5.png`,
  aiReviewFiltered: `${PACKAGE_ASSET}/review_log_evidence/image6.png`,
};
...
const dims = {
  ...
  [images.aiReviewTrace]: [1867, 921],
  [images.aiReviewDecision]: [1866, 919],
  [images.aiReviewFinal]: [1871, 911],
  [images.aiReviewFormal]: [1883, 909],
  [images.aiReviewCandidate]: [1863, 914],
  [images.aiReviewFiltered]: [1881, 909],
};
...
async function createOriginalSlide(presentation, _ctx, number) {
  ...
  if (number === 2) {
    ...
    const cards = [
      ...
      ["AI 辅助提效", "Type8 / Type9 / Type11 / Type12 等模块进入 AI 辅助开发，材料解析脚本完成试点。", C.green],
  ...
  if (number === 3) {
    ...
    const rows = [
      ...
      ["02", "重点工作", "Type9 重构、Type8/11/12 规范化移植、材料结构化、自动验证、AI Review"],
...
function createSmbiosValidationFlowSlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "04 / VALIDATION", "SMBIOS 验证从“人工多环境切换”沉淀为基于 opencode skills 的可追踪自动化流程", page);

  sectionLabel(slide, "旧方式", 72, 188, C.muted);
  card(slide, 72, 226, 270, 126, { fill: C.panel });
  text(slide, "人工刷写 + 手动比对", 100, 250, 190, 24, { size: 19, bold: true, color: C.ink });
  text(slide, "环境切换、dmidecode 执行、日志整理依赖人工串联，过程难复盘。", 100, 290, 190, 42, { size: 14, color: C.slate });

  sectionLabel(slide, "新方式", 72, 400, C.blue);
  card(slide, 72, 438, 270, 142, { fill: C.softBlue });
  text(slide, "opencode + skills", 100, 462, 190, 24, { size: 19, bold: true, color: C.ink });
  text(slide, "将上传、校验、刷写、采集、报告生成串成可重复流程，关键动作留痕。", 100, 502, 190, 48, { size: 14, color: C.slate });

  const steps = ["BIOS 上传", "SHA256 校验", "远程刷写", "人工确认", "系统侧采集", "报告生成"];
  steps.forEach((s, i) => {
    const x = 408 + (i % 3) * 176;
    const y = 232 + Math.floor(i / 3) * 138;
    rect(slide, x, y, 132, 78, { fill: i === 3 ? C.softAmber : C.panel });
    rect(slide, x, y, 132, 6, { fill: i === 3 ? C.amber : C.blue });
    text(slide, String(i + 1).padStart(2, "0"), x + 14, y + 18, 34, 18, { size: 13, bold: true, color: i === 3 ? C.amber : C.blue, font: MONO });
    text(slide, s, x + 14, y + 44, 104, 22, { size: 16, bold: true, color: C.ink });
    if (i % 3 < 2) text(slide, "→", x + 142, y + 28, 24, 24, { size: 20, bold: true, color: C.blue });
    if (i === 2) text(slide, "↓", x + 48, y + 88, 24, 24, { size: 20, bold: true, color: C.blue });
  });

  card(slide, 938, 226, 230, 126, { fill: C.softGreen });
  text(slide, "过程可追踪", 966, 250, 150, 24, { size: 20, bold: true, color: C.ink });
  text(slide, "刷写、采集、报告生成过程保留记录。", 966, 292, 160, 38, { size: 14, color: C.slate });
  card(slide, 938, 394, 230, 126, { fill: C.softAmber });
  text(slide, "结果可复盘", 966, 418, 150, 24, { size: 20, bold: true, color: C.ink });
  text(slide, "dmidecode 输出可比对，便于回看问题差异。", 966, 460, 160, 38, { size: 14, color: C.slate });
  text(slide, "下一页展示自动验证日志和 dmidecode 输出样例。", 408, 598, 520, 26, { size: 16, color: C.muted });
  return slide;
}

function createSmbiosVerifySlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "04 / SMBIOS VERIFY", "SMBIOS 验证日志与 dmidecode 输出已形成可追溯记录", page);
...
function createMaterialStructuringSlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "03 / MATERIAL INPUT", "脚本将客户材料转为结构化输入，补齐内网模型材料读取短板", page);

  sectionLabel(slide, "当前痛点", 78, 190, C.red);
  card(slide, 78, 230, 300, 128, { fill: "#FFF2F2" });
  text(slide, "敏感材料不出内网", 108, 254, 210, 26, { size: 21, bold: true, color: C.ink });
  text(slide, "客户 PPT / Word / 表格包含敏感信息，不能直接交给外网多模态模型处理。", 108, 296, 224, 42, { size: 14, color: C.slate });

  card(slide, 78, 398, 300, 128, { fill: C.panel });
  text(slide, "人工整理效率低", 108, 422, 210, 26, { size: 21, bold: true, color: C.ink });
  text(slide, "逐页阅读、截图比对、字段摘录较繁杂，提取慢且容易漏项。", 108, 464, 224, 42, { size: 14, color: C.slate });
  ...
  flow.forEach((f, i) => {
    const y = 206 + i * 92;
    rect(slide, 474, y, 260, 58, { fill: i === 1 ? C.ink : C.panel });
    text(slide, f[0], 498, y + 10, 120, 20, { size: 17, bold: true, color: i === 1 ? "#FFFFFF" : C.ink });
    text(slide, f[1], 624, y + 9, 86, 34, { size: 12.5, color: i === 1 ? "#DDE7FF" : C.slate });
    if (i < flow.length - 1) text(slide, "↓", 590, y + 62, 24, 24, { size: 18, color: C.blue, bold: true, align: "center" });
  });

  sectionLabel(slide, "试点价值", 856, 190, C.green);
  card(slide, 856, 230, 276, 98, { fill: C.softGreen });
  text(slide, "模型可读", 886, 252, 110, 24, { size: 20, bold: true, color: C.ink });
  text(slide, "把非结构化客户材料转成模型可直接读取的输入。", 886, 288, 192, 28, { size: 13.5, color: C.slate });
  card(slide, 856, 358, 276, 98, { fill: C.softBlue });
  text(slide, "来源可追溯", 886, 380, 130, 24, { size: 20, bold: true, color: C.ink });
  text(slide, "保留页码、表格位置和字段来源，便于工程师复核。", 886, 416, 192, 28, { size: 13.5, color: C.slate });
  card(slide, 856, 486, 276, 98, { fill: C.softAmber });
  text(slide, "后续 Skill 化", 886, 508, 140, 24, { size: 20, bold: true, color: C.ink });
  text(slide, "当前是脚本试点，后续沉淀为 opencode skill。", 886, 544, 192, 28, { size: 13.5, color: C.slate });

  text(slide, "当前定位：脚本先服务规则提取和人工复核，后续再沉淀为内网可调用的 opencode skill。", 78, 620, 900, 26, { size: 16, color: C.muted });
  return slide;
}

function createAiReviewContextSlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "05 / AI REVIEW", "AI Review 把模型分析过程留痕，工程师可以回看它如何判断", page);
  const caps = [
    ["先理解变更", "先记录本次提交涉及的模块、改动范围和潜在影响。", C.softBlue],
    ["再形成线索", "把上下文缺口、初始风险和候选问题统一沉淀。", C.softGreen],
    ["最后给出处置", "每条风险都能看到进入正式、待确认或过滤的原因。", C.softAmber],
  ];
  caps.forEach((c, i) => {
    rect(slide, 72, 194 + i * 112, 334, 84, { fill: c[2] });
    text(slide, `0${i + 1}`, 96, 214 + i * 112, 38, 24, { size: 18, bold: true, color: C.blue, font: MONO });
    text(slide, c[0], 146, 212 + i * 112, 160, 24, { size: 20, bold: true, color: C.ink });
    text(slide, c[1], 146, 246 + i * 112, 212, 28, { size: 13.5, color: C.slate });
  });

  evidence(slide, images.aiReviewTrace, 458, 184, 686, 310, "");
  evidenceCaption(slide, "管理页可回看模型先理解了什么、请求补了哪些上下文、识别了哪些初始风险。", 458, 510, 686);
  ...
  flow.forEach((f, i) => {
    const x = 458 + i * 172;
    rect(slide, x, 558, 136, 52, { fill: f[1] });
    text(slide, f[0], x + 12, 574, 112, 20, { size: 16, bold: true, color: C.ink, align: "center" });
    if (i < flow.length - 1) text(slide, "→", x + 140, 568, 28, 28, { size: 22, bold: true, color: C.blue, align: "center" });
  });
  text(slide, "价值：不只看最终评论，还能解释“为什么没报 / 为什么降级 / 为什么需要人工确认”。", 82, 630, 900, 26, { size: 16, color: C.muted });
  return slide;
}

function createAiReviewRiskSlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "05 / AI REVIEW", "AI Review 将结果分为正式、待确认、已过滤，避免风险全部堆给工程师", page);
  const lanes = [
    ["正式问题", "证据充分，进入修复处理", C.blue],
    ["待确认", "线索有效，等待人工判断", C.amber],
    ["已过滤", "重复、低价值或证据不足，不进入正式评论", C.green],
  ];
  lanes.forEach((lane, i) => {
    const x = 74 + i * 382;
    rect(slide, x, 186, 336, 96, { fill: C.panel });
    rect(slide, x, 186, 8, 96, { fill: lane[2] });
    text(slide, lane[0], x + 28, 208, 130, 26, { size: 22, bold: true, color: C.ink });
    text(slide, lane[1], x + 28, 242, 254, 24, { size: 14, color: C.slate });
  });

  evidence(slide, images.aiReviewFormal, 68, 320, 356, 180, "");
  evidenceCaption(slide, "正式：证据明确的问题进入用户可见 Review。", 68, 512, 356);
  evidence(slide, images.aiReviewCandidate, 462, 320, 356, 180, "");
  evidenceCaption(slide, "待确认：保留有效线索，交给人工复核。", 462, 512, 356);
  evidence(slide, images.aiReviewFiltered, 856, 320, 356, 180, "");
  evidenceCaption(slide, "已过滤：保留排除原因，后续可复盘。", 856, 512, 356);

  rect(slide, 170, 596, 940, 46, { fill: C.ink });
  text(slide, "价值：减少噪音、保留判断依据，让“模型看到了什么”和“最终为什么这么展示”都能被追踪。", 198, 610, 884, 20, {
    size: 16,
    color: "#FFFFFF",
    align: "center",
  });
...
export async function createSlide(presentation, ctx, number) {
  ...
  if (number === 7) {
    return createSpecCodeSlide(presentation, 7);
  }

  if (number === 8) {
    return createTypeRefactorSlide(presentation, 8);
  }

  if (number === 9) {
    return createMaterialStructuringSlide(presentation, 9);
  }

  if (number === 10) {
    return createSmbiosValidationFlowSlide(presentation, 10);
  }

  if (number === 11) {
    return createSmbiosVerifySlide(presentation, 11);
  }

  if (number === 12) {
    return createAiReviewContextSlide(presentation, 12);
  }

  if (number === 13) {
    return createAiReviewRiskSlide(presentation, 13);
  }

  if (number === 14) {
    return createCloseSlide(presentation, 14);
  }
