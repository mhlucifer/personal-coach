...
function createCloseSlide(presentation, page) {
  const slide = slideBase(presentation);
  header(slide, "SUMMARY / NEXT STEP", "本阶段完成客户交付，并沉淀 SMBIOS 场景的 AI 辅助工程流程", page);
  const cards = [
    ["客户交付闭环", "完成 V3 遗留、V5 新增、Type2 SKU、Type11 TDX 等 SMBIOS 适配与阶段性交付。", C.blue],
    ["开发侧流程沉淀", "围绕客户规则、规格文档、代码生成、自动验证和人工验收，形成 SMBIOS 模块可复用路径。", C.green],
    ["下一步探索", "聚焦字段缺失、显示异常、客户可见信息不一致等可观测问题，探索 Agent 辅助生成排查线索。", C.amber],
  ];
  cards.forEach((c, i) => {
    const x = 92 + i * 356;
    rect(slide, x, 196, 300, 246, { fill: C.panel });
    rect(slide, x, 196, 300, 8, { fill: c[2] });
    text(slide, c[0], x + 28, 238, 210, 28, { size: 23, bold: true, color: C.ink });
    text(slide, c[1], x + 28, 300, 230, 94, { size: 16, color: C.slate });
  });
  rect(slide, 92, 506, 1012, 62, { fill: C.ink });
  text(slide, "边界控制", 122, 526, 100, 22, { size: 18, bold: true, color: "#FFFFFF" });
  text(slide, "测试侧定位仅作为工程师排查辅助，保留验证报告、定位记录、处理结果与复测结论，不扩大为通用 BIOS 问题自动修复。", 238, 525, 790, 24, {
    size: 16,
    color: "#FFFFFF",
  });
  text(slide, "探索口径：针对 SMBIOS 可观测差异，沉淀验证记录、关联代码 / 日志线索，评估 AI 辅助缩短首轮排查时间的价值。", 112, 608, 960, 28, {
    size: 16,
    color: C.muted,
  });
  return slide;
